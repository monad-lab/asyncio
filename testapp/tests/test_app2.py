...
@pytest.fixture(scope="session")
async def initialize_test_db(event_loop: asyncio.AbstractEventLoop):
...
@pytest.fixture(scope="session")
async def async_client(initialize_test_db: None, event_loop: asyncio.AbstractEventLoop):
...
@pytest.mark.order(3)
@pytest.mark.asyncio
async def test_create_events(async_client: AsyncClient):
...
@pytest.mark.order(4)
@pytest.mark.asyncio
async def test_get_initial_events(async_client: AsyncClient):
    global initial_events
...
@pytest.mark.order(5)
@pytest.mark.asyncio
async def test_update_events(async_client: AsyncClient):
    actions = ["bet", "raise", "all-in", "check", "fold", "call", "re-raise"]
    for i, token in enumerate(user_tokens):
...
@pytest.mark.order(6)
@pytest.mark.asyncio
async def test_get_updated_events(async_client: AsyncClient):
    headers = {"Authorization": f"Bearer {user_tokens[0]}"}
    response = await async_client.get("/event/get_all_events", headers=headers)
    assert response.status_code == 200  # 이벤트 조회 성공 확인
